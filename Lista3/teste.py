...
from teste_pivot import Pivoteamento
...
mat_inicial_LDLt = np.array([[23.0, 10., 9.0],
                            [10.0, 54.0, 8.0],
                            [9.0, 8.0, 49.0]])
...
def LDLt_decomposition(mat_inicial: np.array):
    mat = np.copy(mat_inicial)
    n = len(mat)
    L = np.identity(n)
    D = np.identity(n)
    ...
    for i in range(n):
        ...
        mat[i, i+1:] /= mat[i, i]
        mat[i+1:, i] /= mat[i, i]
        mat[i+1:, i+1:] -= np.outer(mat[i+1:, i], mat[i, i+1:]) * mat[i, i]
...
print("\nL*U:")
print_matrix(np.round(L @ U, decimals=5))
print("\nMatriz inicial:")
